...
export default function InfiniteShowcaseGrid({
  ...
  if (items.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center rounded-3xl border border-dashed border-accent/30 bg-white/50 px-6 py-24 text-center">
        <span className="text-4xl">🌸</span>
        <p className="mt-4 font-display text-lg font-bold text-ink">
          준비된 페이지가 없어요
        </p>
        <p className="text-sm text-ink-muted/50">다른 업종을 선택해 보세요.</p>
      </div>
    );
  }
  ...
  return (
    <div>
      ...
      <div className="mt-10 flex items-center justify-center py-6">
        {isFetchingNextPage ? (
          <span className="flex items-center gap-2 text-sm text-ink-muted/50">
            <span className="h-4 w-4 animate-spin rounded-full border-2 border-accent/30 border-t-accent" />
            불러오는 중…
          </span>
        ) : !hasNextPage ? (
          <span className="text-sm text-ink-muted/40">전부 봤어요 🌸</span>
        ) : null}
      </div>
    </div>
  );
