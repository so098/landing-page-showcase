...
- [x] '로그인시 개인 내정보 페이지 만들기 - 내정보 페이지 안에는 어떤 랜딩페이지를 만드는지, 랜딩페이지 진행도, 가격, 리뷰작성 버튼이 보여야 함
- [x] 내 정보에서 리뷰작성 버튼 클릭시 모달이 뜨고 리뷰 작성할 수 있어야 함 완료 누르면 토스트로 리뷰를 작성해주셔서 감사합니다 라고 떠야 함
- [x] 랜딩페이지를 완성 후 보는 페이지에서는 환불하기 버튼도 추가되어야 함 모달로 진짜 환불하시겠어요? 환불이유를 작성해주세요라고 떠야 함
- [x] 안내 페이지 안에 아코디언 안에 도메인, 호스팅 표현되어있는 곳에 좀 더 자세히 적어야 함 netlify나 vercel로 호스팅 할것이고 새 계정과 도메인의 최소, 최대 가격대를 주시면 3개 정도의 도메인을 전달 선택해주시면 된다고 하면 될듯
- [x] 안내페이지에 아코디언 안에 추가해줘 seo 최적화란 해가지고 seo를 신경써서 작업합니다 하면서 예시를 들어주고 구글 콘솔 등록, 네이버 콘솔 등록도 해드린다 이정도만 해드린다 하면 될거같은데
- [x] 메타데이터/SEO: 페이지별 `generateMetadata`, OG 태그
- [x] 측정: After 수치 기록 → README에 Before/After 표 (결과: Perf 점수 개선 없음 — 진짜 병목은 렌더 블로킹 폰트. 후속 작업 next/font 전환 도출 — [측정 기록](../../perf/rsc-migration-before-after.md))
...
- [x] Playwright 도입 + E2E: 메인 페이지 로드 → 필터 → 모달 열기 (`e2e/home.spec.ts` — 로드/SSR 증거/필터/모달/콘솔 에러 5케이스)
