#!/usr/bin/env python3
"""
ClipartKorea authenticated asset downloader for landing template folders.
...
MANIFEST_PATH = ASSETS / "clipart-assets-manifest.json"
...
class CKSession:
    ...
    def refresh_csrf(self, keyword: str):
        params = {"keyword": keyword, "menu": "m", "sort": "3", "cate": "krpho,photo"}
    ...
    def headers(self, referer_keyword: str):
        return {
            ...
            "Referer": f"{SEARCH_URL}?" + urllib.parse.urlencode({"keyword": referer_keyword, "menu": "m", "sort": "3", "cate": "krpho,photo"}),
...
def item_score(item: dict) -> int:
    ...
    if has_people and (item.get("contGroup") == "photo") and (kws & FOREIGN_PERSON_MARKERS):
        return -10_000
